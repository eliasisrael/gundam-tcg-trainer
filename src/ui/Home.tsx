    ...
    <div className="home">
      <div className="hero">
        <div>
          ...
          <p className="muted" style={{ maxWidth: 560 }}>Learn the rules by playing them, then practice against a bot with a coach that explains every decision. Built on the official Comprehensive Rules v1.9.0 and the fourteen starter decks ST01-ST14.</p>
        </div>
      ...
      <div className="modes">
        ...
        <button className="mode" onClick={() => onBuilder()}>
          ...
          <h3>Deck Builder</h3>
          <p className="muted">Pick any one or two colors and build a 50-card deck from all fourteen starter pools. Auto-build gives you a legal deck in one click; the color guide explains what each color wants.</p>
      ...
      <p className="muted" style={{ maxWidth: 720 }}>Card pool: the fourteen starter decks ST01-ST14 (plus the Destiny Ignition extras), covering all five colors (Blue, Green, Red, White, Purple).</p>
...
function PracticeSetup({ onBack, onStart, onBuilder }: { onBack: () => void; onStart: (o: PracticeOptions) => void; onBuilder: (deck?: DeckDef) => void }) {
  ...
  return (
    <div className="home">
      ...
      <p className="muted">Fourteen starter decks cover all five colors. Build your own two-color deck in the <span className="link" onClick={() => onBuilder()}>Deck Builder</span> and it appears here for you or the bot.</p>
