...
export const LESSONS: Lesson[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Deploy a Unit', body: 'Click the **GM** in your hand and deploy it. It costs 1, so one Resource rests.', highlight: ['hand'], task: { prompt: 'Deploy GM (cost 1).', check: s => s.players.p1.units.some(u => u.card.defId === 'ST01-005'), hint: 'Click GM in your hand, then choose Deploy.' } },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Pair Amuro', body: 'Now play Amuro Ray (cost 1) onto Gundam. Gundam becomes 5 AP / 5 HP, and Amuro\'s When Paired will let you rest an enemy Unit. Choose the **Tragos**.', highlight: ['hand', 'units'], task: { prompt: 'Pair Amuro Ray with Gundam.', check: s => s.players.p1.units.some(u => u.card.defId === 'ST01-001' && u.pilot?.defId === 'ST01-010') } },
      { title: 'Attack immediately', body: 'Gundam is a **Link Unit** now: it can attack this turn even though you just deployed it. Attack a rested enemy Unit or the player.\n\nBonus: Gundam\'s 【During Pair】 gives **all your Units AP+1** during your turn.', highlight: ['units'], task: { prompt: 'Attack with Gundam.', check: s => s.stats.p1.attacksDeclared >= 1, hint: 'Click Gundam, then pick a target.' } },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Take the free kill', body: 'Your linked Gundam has 6 AP this turn (3 base +2 Amuro +1 from its own During Pair). Attack **Maganac** with Gundam: it dies and its 3 AP does not threaten Gundam\'s 5 HP.', highlight: ['units'], task: { prompt: 'Attack Maganac with Gundam.', check: s => s.players.p2.units.every(u => u.card.defId !== 'ST02-005') } },
      { title: 'Now the Shields', body: 'The opponent has **no Base**, so attacks on the player break Shields directly. Guncannon has 3 AP this turn (2 +1 from Gundam). Attack the player with Guncannon.\n\nWatch: the Shield is revealed. If it has a Burst, the opponent may use it.', highlight: ['units', 'opp-shields'], task: { prompt: 'Attack the player with Guncannon.', check: s => s.players.p2.shields.length <= 5 } },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Bases', body: 'You have White Base in hand (Lv.3, cost 2, **5 HP**). Deploy it: it replaces your EX Base. Its 【Deploy】 adds one of your Shields to your hand, so you are trading a face-down Shield for a real card and a bigger wall.', highlight: ['hand', 'base'], task: { prompt: 'Deploy White Base.', check: s => s.players.p1.base?.card.defId === 'ST01-015' } },
      ...
      { title: 'Targeting', body: 'Thoroughly Damaged needs a **rested** enemy Unit. The Leo is rested, so it is a legal play (1 damage). Unforeseen Incident targets any enemy Unit.\n\nPlay Thoroughly Damaged on the Leo.', highlight: ['hand', 'opp-units'], task: { prompt: 'Play Thoroughly Damaged targeting Leo.', check: s => s.players.p2.units.some(u => u.card.defId === 'ST02-007' && u.damage >= 1) || !s.players.p2.units.some(u => u.card.defId === 'ST02-007') } },
